class ConclusionNivel9 extends Phaser.Scene {
  constructor() {
    super({ key: "ConclusionNivel9" });
  }

  preload() {
    // Load video with better configuration to avoid range request issues
    this.load.video("video4", "assets/video4/Final.mp4", "canplaythrough");
    ...
    this.load.on("loaderror", (file) => {
      if (file.key === "video4") {
        console.error("Error loading video4:", file);
        // Try alternative loading method
        console.log("Attempting alternative video loading...");
        this.load.video(
          "video4_retry",
          "assets/video4/Final.mp4",
          "loadedmetadata"
        );
      }
    });
  ...
  create() {
      ...
      if (videoElement.error?.code === 3) {
        ...
        setTimeout(() => {
          try {
            ...
            if (
              recoveryPromise &&
              typeof recoveryPromise.catch === "function"
            ) {
    ...
    videoElement.addEventListener("waiting", () => {
      waitingCount++;
      if (waitingCount <= 3) {
        // Only log first few waiting events to reduce console spam
    ...
    videoElement.addEventListener("loadedmetadata", () => {
      ...
      console.log(
        `🎬 Video: loadedmetadata event fired - Duration: ${videoDuration} seconds`
      );
      if (videoDuration && !isNaN(videoDuration)) {
        console.log(
          `🎬 Video: Setting fallback timeout for ${videoDuration + 2} seconds`
        );
        // Set a timeout slightly longer than video duration as fallback
        setTimeout(() => {
          if (this.scene.isActive("ConclusionNivel9")) { // Fixed: was checking TransicionNivel9 instead of ConclusionNivel9
            console.log(
              "⏰ Fallback timeout triggered - transitioning to next scene"
            );
            if (audioManager) {
            ...
            this.scene.start("Ultima"); // Standardized to Ultima scene
          }
    ...
    videoElement.addEventListener("timeupdate", () => {
      ...
      if (duration && currentTime) {
        ...
        if (currentTime % 5 < 0.1) {
          // Log every 5 seconds
          console.log(
            `🎬 Video progress: ${progress}% (${currentTime.toFixed(
              1
        ...
        if (duration - currentTime < 1) {
          console.log(
            "🎬 Video: Near end detected, preparing for transition..."
          );
        }
      }
    });
    ...
    video.on("complete", () => {
      console.log(
        "🎬 Phaser Video: complete event triggered - transitioning to next scene"
      );
      // Reanudar la música antes de cambiar de escena
      if (audioManager) {
        audioManager.resumeMusic();
...
window.ConclusionNivel9 = ConclusionNivel9;
